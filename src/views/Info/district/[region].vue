...
<template>
  <div class="w-full flex flex-col gap-4 overflow-x-hidden">
    <TheBreadcrumb
      :home="store.user?.role === 'WORKER' ? null : '/info/district'"
      :items="[
        ...(store.user?.role === 'WORKER' ? [] : [{ label: 'Viloyatlar', link: '/info/district' }]),
              ...
              @click="
                () => {
                  if (
                    ...
                    router.push({
                      path: `/info/district/${route.params.region}/${record.district_id}`,
                      query: {
                        ...route.query,
    ...
    <div class="text-left">
      <div
        ...
        :class="{
          'text-cBlue font-bold cursor-pointer underline': 
            store.canAccess({ region_id: record.region_id }) && 
        ...
        @click="() => {
          if (store.canAccess({ region_id: record.region_id }) && 
            ...
            router.push({
              ...
              query: { ...route.query, region: record.region_name }
            });
          }
        }"
      >
...
::v-deep(th:nth-child(2)) {
  background-color: #C7D0E2 !important; /* Hudud nomi ustuni */
}
