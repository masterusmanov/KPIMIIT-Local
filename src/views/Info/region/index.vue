...
const fetchData = async () => {
  try {
    ...
    const processChildren = (item: any, index: number) => {
      if (item.children && item.children.length > 0) {
        return item.children.map((child: any) => {
          ...
          // Process next level of children if they exist
          if (child.children && child.children.length > 0) {
            processedChild.children = processChildren(child, index);
          } else {
    ...
    columns.value = [
      ...
      ...res.data.data.columns.map((item: any, index: number) => ({
        ...
        width: 120,
        align: "center",
        children: processChildren(item, index),
        customHeaderCell: () => ({
          class: `group-${index}`,
        }),
      })),
